  ...
  try {
    ...
    try {
      ...
      if (profile) {
        ...
        // Still return success since subscription order was saved
        // Frontend can refresh profile to get updated subscription
        return res.status(200).json({
          ...orders.toObject(),
          subscriptionStartDate: subscriptionStartDate,
          subscriptionExpiryDate: subscriptionExpiryDate,
          message: `Subscription upgraded to ${subscriptionType} for 1 month`,
          warning: 'Profile not found, but subscription order was saved. Please refresh your profile.',
        });
      }
    } catch (profileError: any) {
      console.error('Error updating profile subscription:', profileError);
      // Log the error but still return success since subscription order was saved
      // Frontend can refresh profile to get updated subscription
      profileUpdateSuccess = false;
    }

    // Return success regardless - subscription order is saved
    // Frontend will refresh profile to get updated subscription
    return res.status(200).json({
      ...orders.toObject(),
      subscriptionStartDate: subscriptionStartDate,
      subscriptionExpiryDate: subscriptionExpiryDate,
