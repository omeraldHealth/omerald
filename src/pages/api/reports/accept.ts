...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const url = `${DC_API_BASE_URL}/api/reports/accept`;
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error?.message || errorData.message || `Failed to accept report: ${response.statusText}`);
    }
    ...
    console.error('Error accepting report from DC:', error);
    return res.status(500).json({
      success: false,
      error: {
        code: 'INTERNAL_ERROR',
        message: error.message || 'Failed to accept report from DC',
      },
    });
  }
};
