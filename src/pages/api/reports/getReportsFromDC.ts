...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    // Fetch reports from DC API
    const response = await fetch('https://omerald-dc.vercel.app/api/reports');
    ...
    // Filter reports where sharedReportDetails contains the phone number
    ...
    const filteredReports = reportsArray.filter((report: any) => {
      ...
      return report.sharedReportDetails.some((detail: any) => {
        ...
        // Include reports that match the user's phone number and are not blocked
