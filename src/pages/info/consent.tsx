...
const Consent = () => {
  const { platformConsentHtml, isLoading, isError } = useUserSettings();
            ...
            <div className="w-full px-3">
              <div className="pb-12">
                <div className="max-w-7xl mx-auto lg:mx-0 mb-8 text-center lg:text-left">
                  {isLoading && (
                    <p className="text-gray-600">Loading platform consent…</p>
                  )}
                  {isError && (
                    <p className="text-amber-700">Could not load content. Please try again later.</p>
                  )}
                  {!isLoading && !isError && platformConsentHtml && (
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: platformConsentHtml }}
                    />
                  )}
                  {!isLoading && !isError && !platformConsentHtml && (
                    <p className="text-gray-600">No platform consent content available.</p>
                  )}
                </div>
              </div>
            </div>
