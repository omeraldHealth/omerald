...
export default function Articles() {
  ...
  // Add CSS animation for fade-in-up effect - must be before any early returns
  React.useEffect(() => {
  ...
  const toggleHealthTopic = (topicId: number) => {
    setSelectedHealthTopicIds(prev => {
      const newIds = prev.includes(topicId) 
        ...
        <div className="grid grid-cols-12 gap-6 lg:gap-8 xl:gap-10">
            ...
            <div className="bg-white p-6 xl:p-8 rounded-xl shadow-sm border border-gray-200">
              ...
              {loadingHealthTopics ? (
                ...
                <>
                      ...
                      return (
                        <button
                          ...
                          className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 ease-in-out flex items-center gap-2 transform hover:scale-105 ${
                            isSelected
                              ? 'bg-blue-600 text-white shadow-md hover:bg-blue-700 ring-2 ring-blue-300'
                              : 'bg-gray-100 text-gray-700 hover:bg-gray-200 hover:shadow-sm'
                          }`}
                        >
                          {topic.title}
                          <span className={`text-xs transition-opacity duration-200 ${isSelected ? 'opacity-90' : 'opacity-75'}`}>
                            ({topic.article_count})
                          </span>
                          {isSelected && (
                  ...
                  <div className="mt-4 text-sm text-gray-600 pt-4 border-t border-gray-200 transition-all duration-300">
                    {loading ? (
                      <div className="flex items-center gap-2">
                        <div className="w-4 h-4 border-2 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                        <span>Filtering articles...</span>
                      </div>
                    ) : (
                      <span>
                        Showing <span className="font-semibold text-blue-600 transition-colors duration-300">{totalCount}</span> article{totalCount !== 1 ? 's' : ''} for selected topics
                      </span>
                    )}
                  </div>
                </>
          ...
          <main className="col-span-12 lg:col-span-8 relative">
            ...
            {loading && articles.length > 0 && (
              <div className="absolute inset-0 bg-white bg-opacity-60 backdrop-blur-sm z-10 flex items-center justify-center rounded-xl transition-opacity duration-300">
                <div className="flex flex-col items-center gap-3">
                  <div className="w-10 h-10 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                  <div className="text-gray-600 text-sm font-medium">Updating articles...</div>
                </div>
              </div>
            )}
            
            {error && (
              <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg text-red-700 animate-fadeIn">
                {error}
              </div>
            )}
            ...
            {!hasHealthData ? (
              ...
              <div className="text-center py-16 bg-white rounded-xl shadow-sm border border-gray-200 animate-fadeIn">
                <Search className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                <p className="text-gray-500 text-lg font-medium">No articles found</p>
                <p className="text-gray-400 text-sm mt-2">
                  {selectedHealthTopicIds.length === 0 
                    ? 'Select health topics to see articles'
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 xl:gap-6">
                {loading && filteredArticles.length === 0 && articles.length === 0 ? (
                  <div className="col-span-2 flex items-center justify-center py-16">
                    <div className="flex flex-col items-center gap-4">
                      <div className="w-12 h-12 border-4 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
                      <div className="text-gray-500 text-lg">Filtering articles...</div>
                    </div>
                  </div>
                ) : (
                  filteredArticles.map((article, index) => (
                    <article
                      key={article.id}
                      onClick={() => handleArticleClick(article)}
                      className="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all duration-500 overflow-hidden group cursor-pointer flex flex-col"
                      style={{
                        animation: `fadeInUp 0.5s ease-out ${Math.min(index * 30, 300)}ms both`,
                      }}
                    >
