...
function UserMembers() {
  ...
  const filteredAndSortedMembers = useMemo(() => {
    ...
    // Sort - always put Self first, then apply other sorting
    filtered = [...filtered].sort((a, b) => {
      // Always put Self first
      const aIsSelf = a.relation === 'Self';
      const bIsSelf = b.relation === 'Self';
      if (aIsSelf && !bIsSelf) return -1;
      if (!aIsSelf && bIsSelf) return 1;
      
      // If both are Self or both are not Self, apply normal sorting
      if (sortBy === 'name') {
