...
const Profile = ({ member: memberProp, onBack }: ProfileProps = {}) => {
  ...
  // Prefetch vaccine data when profile loads (if pediatric) or when immunization tab is about to be viewed
  useEffect(() => {
    if (isPediatric && profileData) {
      // Prefetch vaccine data in the background for instant loading
      queryClient.prefetchQuery({
        queryKey: ['allVaccineData'],
        queryFn: async () => {
          const { getAllVaccineData } = await import('@/services/omeraldAdminService');
          return await getAllVaccineData();
        },
      });
    }
  }, [isPediatric, profileData, queryClient]);

  // Prefetch when hovering over or clicking immunization tab
  const handleTabChange = useCallback((tab: TabType) => {
    setActiveTab(tab);
    // Prefetch vaccine data when user clicks on immunization tab
    if (tab === 'immunization' && isPediatric) {
      queryClient.prefetchQuery({
        queryKey: ['allVaccineData'],
        queryFn: async () => {
          const { getAllVaccineData } = await import('@/services/omeraldAdminService');
  ...
  useEffect(() => {
    ...
    if (reports && reports.length > 0 && !isAnalyzingConditions) {
      ...
      if (shouldAnalyze) {
        ...
        // COMMENTED OUT: Hide intelligent scan alert
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    onMouseEnter={() => !isDisabled && handleTabHover(tab.id)}
                    disabled={isDisabled}
