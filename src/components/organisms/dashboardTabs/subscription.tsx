        ...
        handler: async function (response: any) {
          try {
            ...
            if (result.status === 200 && result.data) {
              ...
              // Refresh profile to get updated subscription (always refresh to ensure sync)
              try {
                await refreshProfile();
              } catch (refreshError) {
                console.error('Error refreshing profile after subscription update:', refreshError);
                // Don't show error - profile will be refreshed on next page load
              }
              ...
              try {
                ...
                // Don't show error - queries will refresh on next render
              }
              ...
              // If status is not 200, check if subscription order was saved
              if (result.data?.subscriptionStartDate) {
                // Subscription order was saved, just refresh profile
                toast.success('Payment successful! Refreshing your subscription...', { duration: 3000 });
                await refreshProfile();
                setTimeout(() => {
                  setLoading(null);
                }, 1000);
            ...
            // Check if the error response indicates subscription was saved
            if (error?.response?.status === 200 || (error?.response?.data && !error?.response?.data?.error)) {
              // Subscription was saved, just refresh profile
              toast.success('Payment successful! Refreshing your subscription...', { duration: 3000 });
              try {
                await refreshProfile();
                setTimeout(() => {
