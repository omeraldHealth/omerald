...
export default function CameraCapture({
  ...
  return (
    <div className="fixed inset-0 bg-black z-50 flex flex-col">
      {/* Header */}
      <div className="bg-black/80 text-white p-4 flex items-center justify-between z-10">
        <button
          onClick={() => {
            stopCamera();
            onClose();
      ...
      {/* Controls */}
      <div className="bg-black/80 p-4 sm:p-6 flex flex-col items-center justify-center gap-4">
        <div className="flex items-center justify-center gap-4">
          {capturedPhotos.length < maxPhotos && stream && isVideoReady && (
            <button
              onClick={capturePhoto}
              disabled={isCapturing || !isVideoReady}
              className="w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-lg hover:bg-gray-100 active:scale-95 transition-all disabled:opacity-50 touch-manipulation"
        ...
        {capturedPhotos.length === 0 && stream && isVideoReady && (
