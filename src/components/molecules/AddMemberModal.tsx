...
export default function AddMemberModal({ visible, setVisible, profile, onMemberAdded, editingMember, onMemberUpdated }: AddMemberModalProps) {
  ...
  useEffect(() => {
    if (formData.dob) {
      ...
      // Check if age < 2 (including negative ages for future dates or very recent births)
      if (age !== null && age < 2) {
        // Always auto-check pediatric if age < 2 years
        setFormData(prev => ({ ...prev, isPediatric: true }));
      }
    }
  }, [formData.dob]);
  ...
  const handleUpdateMember = async (e: React.FormEvent) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['getProfileByPhoneNumber'] });
      queryClient.invalidateQueries({ queryKey: ['getMembersById'] });
      queryClient.invalidateQueries({ queryKey: ['getProfileById', editingMember._id] });
            ...
            <form onSubmit={isEditMode ? handleUpdateMember : handleCreateAndAdd} className="space-y-4 sm:space-y-6">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      // Calculate age immediately and set isPediatric accordingly
                      let newIsPediatric = formData.isPediatric;
                      if (newDob) {
                        const age = calculateAge(newDob);
                        if (age !== null && age < 2) {
                          // Force isPediatric to true if age < 2
                          newIsPediatric = true;
                        }
                        // Note: We don't automatically uncheck if age >= 2, to preserve user's choice
                      } else {
                        // If DOB is cleared, reset isPediatric
                        newIsPediatric = false;
                      }
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                {(() => {
                  ...
                  const isAgeLessThan2 = age !== null && age < 2;
                  const isDisabled = isAgeLessThan2;
                  // Force checked to true if age < 2, otherwise use formData.isPediatric
                  const isChecked = isAgeLessThan2 ? true : formData.isPediatric;
                  
                  return (
                    <label className={`flex items-center gap-3 ${isDisabled ? 'cursor-not-allowed opacity-75' : 'cursor-pointer'}`}>
                      <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={(e) => {
                          if (!isDisabled) {
                            setFormData({ ...formData, isPediatric: e.target.checked });
                          }
                        }}
                        disabled={isDisabled}
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                          Enable this to access pediatric-specific features: MUAC, Anthropometric, Immunization Schedule, and IAP Growth Charts
                          {isDisabled && (
                            <span className="block mt-1 text-blue-700 font-medium">
                              Automatically enabled for patients under 2 years of age
                            </span>
                          )}
                        </p>
                      </div>
