  ...
  const filteredAndSortedMembers = React.useMemo(() => {
    ...
    if (sortField && sortDirection) {
      if (sortField === 'name') {
        ...
        filtered = orderBy(filtered, ['dob'], [sortDirection]);
      } else if (sortField === 'relation') {
        filtered = orderBy(filtered, [(member) => (member.relation || '').toLowerCase()], [sortDirection]);
      } else if (sortField === 'isPediatric') {
        filtered = orderBy(filtered, ['isPediatric'], [sortDirection === 'asc' ? 'asc' : 'desc']);
      }
    }
